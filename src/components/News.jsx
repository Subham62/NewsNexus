...
//       let url = `https://newsdata.io/api/1/latest?apikey=pub_bcc945f2f52d460fae9410718646e8d9&country=in&category=${this.props.category}&page=${this.state.page}&pageSize=${this.props.pageSize}`;
...
//     return (
//       <>
//         <h1 className='text-center' style={{margin: '40px 0px'}}>NewsNexus - Top {this.capitalizeFirstLetter(this.props.category)} Haedlines</h1>
...
export class News extends Component {
  ...
  async updateNews(){
    this.props.setProgress(10);
    let url = `https://newsdata.io/api/1/latest?apikey=pub_bcc945f2f52d460fae9410718646e8d9&country=${this.props.country}&category=${this.props.category}&language=en`;
    ...
    try {
      ...
      if (parseData.results && Array.isArray(parseData.results)) {
        this.setState({
          ...
          filteredArticles: parseData.results,
          nextPage: parseData.nextPage,
          loading: false
        });
      } else {
        console.error("API Error or No Results:", parseData);
        this.setState({
          articles: [],
          filteredArticles: [],
          loading: false
  ...
  fetchMoreData = async() => {
    ...
    let url = `https://newsdata.io/api/1/latest?apikey=pub_bcc945f2f52d460fae9410718646e8d9&country=${this.props.country}&category=${this.props.category}&language=en&page=${this.state.nextPage}`;
    
    try {
      let data = await fetch(url);
      let parseData = await data.json();
      console.log("Load More Response:", parseData);
      
      if (parseData.results && Array.isArray(parseData.results)) {
        this.setState({
          articles: this.state.articles.concat(parseData.results),
          filteredArticles: this.state.articles.concat(parseData.results),
          nextPage: parseData.nextPage,
          loading: false
        });
      }
  ...
  render() {
    return (
      <>
        <h1 className='text-center' style={{margin: '40px 0px 20px 0px'}}>
          {this.props.category !== "top" 
            ? `NewsNexus - Top ${this.capitalizeFirstLetter(this.props.category)} Headlines` 
            : `NewsNexus - Top Headlines`}  
        </h1>
        ...
        {!this.state.loading && this.state.filteredArticles.length === 0 && (
        ...
        {!this.state.loading && this.state.filteredArticles.length > 0 && (
          <InfiniteScroll
            dataLength={this.state.filteredArticles.length}
            next={this.fetchMoreData}
            hasMore={this.state.nextPage !== null && this.state.searchQuery === ''}
            loader={<Spinner/>}
          >
            <div className='container'>
              <div className="row">
                {this.state.filteredArticles.map((element, index)=>{
                  return <div className="col-md-3 mb-4" key={element.article_id || index}>
                    <NewsItem  
                      title={element.title || "No Title"} 
                      description={element.description || element.content || "No description available"} 
                      imageUrl={element.image_url || "https://via.placeholder.com/300x200?text=No+Image"} 
                      newsUrl={element.link} 
                      author={element.creator ? element.creator[0] : "Unknown"} 
                      date={element.pubDate} 
                      source={{name: element.source_id}}
